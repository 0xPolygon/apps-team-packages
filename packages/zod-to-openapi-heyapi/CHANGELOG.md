...
- [#83](https://github.com/0xPolygon/apps-team-packages/pull/83) [`340bdb2`](https://github.com/0xPolygon/apps-team-packages/commit/340bdb2d7edc2843703cc6640ab4abb29d3bc3ef) Thanks [@MaximusHaximus](https://github.com/MaximusHaximus)! - `schemasFrom` now resolves from the consumer's perspective — `'#schemas'` aliases work
...
- [#81](https://github.com/0xPolygon/apps-team-packages/pull/81) [`c514a10`](https://github.com/0xPolygon/apps-team-packages/commit/c514a10d50f04f90c680dd6438df071fadb7c97d) Thanks [@MaximusHaximus](https://github.com/MaximusHaximus)! - Report 2xx bodies that fail response validation as `ResponseValidationError` instead of `TransportError`
  ...
  ## Docs
  - `schemasFrom: '#schemas'` (a `package.json#imports` alias) was documented but has never worked: Node resolves `#` aliases against the package containing the importing module, and the plugin dynamic-imports `schemasFrom` from its own install location, where the consumer's alias is not defined. As of 2.1.0 the alias resolves correctly (the audit anchors resolution at the codegen output directory), and `'#schemas'` is the canonical same-package pattern — see the README.
