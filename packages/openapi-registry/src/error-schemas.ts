/**
 * Canonical Zod schemas for the error response shapes that
 * `@polygonlabs/express`'s registry-driven router (in concert with
 * `createErrorHandler`) emits. Consumers reference these from their
 * `responses[code].content` slots so the OpenAPI spec accurately documents
 * what clients will actually see — with no copy-pasted-and-drifting
 * per-service definitions.
 *
 * They live in `@polygonlabs/openapi-registry` (rather than
 * `@polygonlabs/express`) so schemas-only packages can register the
 * canonical 400 / 401 / 5xx response shapes without taking a transitive
 * dep on Express + pino + Sentry. `@polygonlabs/express/registry`
 * re-exports them for back-compat with the previous import path.
 *
