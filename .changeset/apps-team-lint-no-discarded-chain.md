...
Add `polygon/no-discarded-chain` rule to the `typescript()` preset.
...
To opt out for a deliberate test fixture (e.g. demonstrating the failure mode), add a scoped `eslint-disable polygon/no-discarded-chain` directive with a `--` comment explaining why.
