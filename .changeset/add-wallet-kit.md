...
Consumer apps still own app-specific concerns (chain catalogs, Redux or context shape, modals, analytics, debug mock addresses, transaction flows, route filtering, copy). External-SCW disclaimer modals are app-owned: read `isExternalSmartContractWallet` from `usePolygonWallet()` and manage dismissal state in the modal component. See the package README for the supported provider and hook surface.
