...
1. Create `packages/<name>/` — at minimum: `package.json`, the three-tier
   `tsconfig.json` / `tsconfig.lib.json` / `tsconfig.spec.json` (copy from the
   closest existing package), and `eslint.config.js`
